...
class KotakNiftyAPI:
    ...
    def get_option_chain(self, index: str, expiry: str, strike_count: int = 10):
        ...
        try:
            ...
            if spot == 0: 
                return {"success": False, "message": f"Failed to get {index} Spot"}
            ...
            for _, row in df.iterrows():
                try:
                    ...
                    print(f"ERROR row mapping for {index}: {e}")
                    continue
            ...
            start = max(0, idx - strike_count//2)
            end = min(len(all_strikes), idx + strike_count//2 + 1)
            ...
            q_data = {}
            if slugs:
                q_url = f"{self.session_data['base_url']}/script-details/1.0/quotes/neosymbol/{','.join(slugs)}"
...
async def chain_api(expiry: str, strikes: int = 10):
    return kotak_api.get_nifty_option_chain(expiry, strikes)

@app.get("/api/option-chain-v2")
async def chain_v2_api(expiry: str, index: str = Query("NIFTY"), strikes: int = 10):
